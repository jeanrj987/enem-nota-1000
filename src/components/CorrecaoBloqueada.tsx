  ...
  return (
    <div className="space-y-6">
      <div className="glass-panel rounded-3xl border border-slate-800 p-6 sm:p-8">
        ...
        <p className="mt-1 text-sm text-slate-400">Tema: {redacao.tema}</p>

        {chamariz.anulada && (
          <div className="mt-5 flex items-start gap-3 rounded-xl border border-red-800/60 bg-red-950/40 p-4">
      ...
      <div className="relative">
        <div className="pointer-events-none select-none space-y-4 opacity-40 blur-[6px]" aria-hidden>
          {[...Array(4)].map((_, i) => (
            <div key={i} className="rounded-2xl border border-slate-800 bg-slate-900/60 p-6">
              ...
              <div className="mb-2 h-2.5 w-11/12 rounded bg-slate-800" />
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center gap-4 px-6 text-center">
          <div className="flex h-14 w-14 items-center justify-center rounded-2xl border border-blue-800/60 bg-blue-950/80 text-blue-400">
            <Lock className="h-7 w-7" />
          </div>
          <div className="max-w-md space-y-1">
            <h2 className="text-lg font-bold text-white">Sua correção completa está pronta</h2>
          ...
          <Link
            href="/vendas"
            className="inline-flex items-center gap-2 rounded-xl bg-blue-600 px-7 py-3 text-sm font-bold text-white shadow-lg shadow-blue-600/30 transition-colors hover:bg-blue-500"
          >
            Ver planos
