...
export function AbaPlano({ pontosPositivos, proximosPassos }: AbaPlanoProps) {
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
      <div className="glass-panel p-6 sm:p-8 rounded-xl border border-regua space-y-4">
        ...
        <ul className="space-y-3 text-xs sm:text-sm text-tinta-suave">
          {pontosPositivos?.map((ponto, idx) => (
            <li key={idx} className="flex items-start gap-2.5 p-3 rounded-xl bg-folha/60 border border-regua">
      ...
      <div className="glass-panel p-6 sm:p-8 rounded-xl border border-regua space-y-4">
        <h3 className="text-base font-bold text-azul flex items-center gap-2">
        ...
        <ul className="space-y-3 text-xs sm:text-sm text-tinta-suave">
          {proximosPassos?.map((passo, idx) => (
            <li key={idx} className="flex items-start gap-2.5 p-3 rounded-xl bg-folha/60 border border-regua">
              <span className="w-5 h-5 rounded-full bg-azul-claro text-azul flex items-center justify-center shrink-0 text-xs font-bold mt-0.5">
                {idx + 1}
              </span>
              <span>{passo}</span>
            </li>
