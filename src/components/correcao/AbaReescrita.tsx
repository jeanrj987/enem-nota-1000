  ...
  return (
    <div className="glass-panel p-6 sm:p-8 rounded-xl border border-regua space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-regua pb-4">
        ...
        <button
          onClick={onCopy}
          className="flex items-center gap-2 px-4 py-2 rounded-xl bg-folha-2 hover:bg-pauta text-tinta text-xs font-semibold border border-regua transition-all self-start sm:self-auto cursor-pointer"
        >
      ...
      <div className="p-6 rounded-xl bg-papel/80 border border-regua font-sans leading-relaxed text-tinta text-sm whitespace-pre-wrap space-y-4">
        {versaoReescrita}
      </div>
    </div>
  );
