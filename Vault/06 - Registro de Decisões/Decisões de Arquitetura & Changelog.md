...
- **Testes**: 182 (sem variação — mudança de conteúdo e de navegação, sem lógica nova testável; os eventos de medição têm cobertura no ADR 042).

### ADR 042: Medição de funil e receita — Meta Pixel no navegador e Conversions API no servidor
- **Status**: Aprovado e Implementado. **Pendente de configuração** (variáveis de ambiente e painéis).
- **Contexto**: o projeto não tinha medição nenhuma. Sem Analytics, sem pixel, sem funil — a única instrumentação era `logCorrecao` (`observabilidade.ts`), que cobre a pipeline de correção, nada de negócio. Com tráfego pago rodando, isso significa não saber quanto custa uma venda.
- **O problema que define a arquitetura**: o checkout da Kiwify roda em `pay.kiwify.com.br`, outro domínio. **O Pixel do navegador nunca vê a venda acontecer** — ele perde a pessoa no clique em "assinar". Um Pixel instalado sozinho registraria visitas e cliques e nenhuma compra: o Meta não conseguiria otimizar por "Compra", e o custo por aquisição no painel ficaria vazio ou errado.
- **Decisão — duas metades, ambas obrigatórias**:
  1. **Guardar a atribuição antes de a pessoa sair.** O `fbclid` que vem na URL do anúncio é capturado e gravado como cookie `_fbc` por código nosso (`src/lib/analytics/atribuicao.ts`), e não pelo Pixel: o Pixel só grava esse cookie se estiver carregado no instante exato da chegada, e bloqueador ou carregamento lento fazem perder a captura. No clique do checkout, `_fbc` e `_fbp` vão para `public.atribuicao_anuncio` (`supabase/schema-atribuicao.sql`) via `/api/atribuicao`.
  2. **Mandar a compra pelo servidor.** Quando o webhook confirma o pagamento, `registrarCompraNoMeta` envia `Purchase` à Conversions API com o `_fbc` guardado (`meta-capi.ts`). Vai pelo servidor porque bloqueador de anúncio e o ITP do Safari derrubam boa parte dos eventos de navegador — e essa fatia não é aleatória, é concentrada em quem compra pelo celular.
- **Deduplicação por `event_id` derivado do pedido** (`compra_<order_id>`), não sorteado: reentrega de webhook, reprocessamento, ou uma compra ativada pelo resgate de órfã (ADR 040) chegam ao Meta com o mesmo id e contam **uma** conversão. Id aleatório inflaria o número de vendas e faria o custo por aquisição parecer menor do que é — o erro que leva a escalar campanha que não se paga. Por isso a compra é reportada nos dois pontos em que o acesso pago nasce (webhook e `vincularCompraOrfa`) sem risco de contagem dupla.
- **Armadilhas do formato tratadas e cobertas por teste**: o e-mail vai em SHA-256 **normalizado** (minúsculas, sem espaços), ou a correspondência com o usuário do Facebook falha; `fbc` e `fbp` **não** são hasheados, por serem identificadores do próprio Meta — hasheá-los é o erro silencioso clássico, em que o evento é aceito e a atribuição some.
- **Eventos do funil**: `ViewContent` (landing), `CompleteRegistration` (conta criada), `Lead` (redação enviada), `InitiateCheckout` (clique para pagar) e `Purchase` (servidor). São nomes **padrão** do Meta de propósito: evento customizado não aparece na lista de objetivos de otimização do Gerenciador de Anúncios. O GA4 recebe os mesmos momentos com os nomes dele (`sign_up`, `begin_checkout`, `purchase`), para os relatórios prontos funcionarem sem configuração manual.
- **Tudo opcional por design**: sem as variáveis, as funções viram no-op silencioso e `Medicao` não renderiza nada. Toda chamada externa é `try/catch` com timeout de 5s — bloqueador de anúncio deixando `window.fbq` indefinido é o cenário mais comum do mundo real, e medição quebrada nunca pode derrubar a venda que ela existe para contar.
- **Não ligar o pixel do Meta dentro do painel da Kiwify**: se a Kiwify disparar o próprio `Purchase`, ele virá sem o nosso `event_id` e o Meta não terá como deduplicar — a venda conta duas vezes. A compra deve ser reportada só pela nossa Conversions API.
- **LGPD**: `SUBPROCESSADORES` (`src/lib/controlador.ts`) ganhou Meta e Google, e **corrigiu uma declaração falsa**: ainda listava a Stripe, removida do projeto no ADR 028, e omitia a Kiwify, que é quem de fato recebe os dados de cobrança. `VERSAO_DOCUMENTOS_LEGAIS` foi para `2026-09-18`. Não há banner de consentimento — coerente com o ADR 019, em que o usuário decidiu manter só a Política de Privacidade. **Fica registrado como risco conhecido**: a persona é vestibulando, muitos menores de idade, e marketing com dado de menor é o ponto mais sensível da LGPD.
- **Testes**: 171 → 182 (11 novos em `meta-capi.test.ts`: normalização e hash do e-mail, ausência de hash em `fbc`/`fbp`, `event_id` preservado, token fora do corpo, e os três caminhos de falha — recusa do Meta, rede caída e timeout).

### ADR 041: `robots.txt` e `sitemap.xml` gerados pelo App Router
- **Status**: Aprovado e Implementado.
- **Contexto**: o site não tinha nem um nem outro. O `metadataBase` e o link canônico já existiam desde o ADR 032, mas faltava dizer ao buscador o que rastrear e o que ignorar.
- **Decisão**: `src/app/robots.ts` e `src/app/sitemap.ts` (convenções de arquivo do App Router, geradas em build), ambos lendo `urlDoSite()` para não cravar domínio no código.
- **O sitemap tem duas URLs, e é para ser assim**: `/` e `/privacidade`. Todo o resto é área logada, endpoint ou redirect. `/vendas` ficou **de fora** de propósito — responde 308 para `/`, e declarar um redirect no sitemap contradiz o canônico e gasta rastreamento para o buscador redescobrir que o endereço bom é a raiz.
- **Registrado dentro do próprio arquivo, para não criar falsa segurança**: `Disallow` não protege nada (quem protege as rotas internas é o gate de sessão e a RLS) e não é `noindex` (uma URL bloqueada ainda pode aparecer no buscador se linkada de fora). O ganho real é não gastar rastreamento em tela de app que, para um visitante deslogado, é só um redirect.
- **Testes**: 182 (sem variação — saída verificada com `next start`: `robots.txt` e `sitemap.xml` servidos com o domínio próprio já resolvido).

### ADR 040: Compra que não encontra conta vira fila de resgate com autoatendimento, não mais um `console.error`
- **Status**: Aprovado e Implementado.
- **Contexto**: o ADR 028 registrou, como **risco aceito conscientemente**, que uma compra feita com e-mail diferente do cadastro não seria vinculada automaticamente — "fica registrada no painel da Kiwify para ativação manual, sem tela de suporte dedicada (decisão explícita para não construir complexidade para um caso raro agora)". A revisão de 18/09 mostrou que o caso não é raro e que o custo dele foi subestimado: o campo de e-mail do checkout da Kiwify é **editável**, e paga-se com o e-mail do cartão, com o do pai ou da mãe, ou com o mesmo e-mail e um erro de digitação. O prefill `?email=` implementado na landing reduz a chance, mas não impede.
- **O que de fato acontecia**: `buscarUserIdPorEmail` devolvia `null`, o webhook gravava um `console.error` e respondia **`200 OK`**. O `200` é o problema estrutural — para a Kiwify deu tudo certo e ela não reenvia. Resultado: dinheiro cobrado, acesso não liberado, nenhum alerta, nenhuma fila, nenhum painel. O único mecanismo de detecção era o aluno reclamar; quem não reclamava virava reembolso, chargeback ou sumia. Somava-se a isso o destino natural de quem estava nessa situação: `/checkout/sucesso` desistia depois de 12 segundos e mandava "recarregue esta página em instantes" — instrução que nunca resolveria, porque o webhook já tinha passado.
- **Decisão**: toda compra **aprovada** que não termina em acesso liberado passa a gravar uma linha em `public.compras_orfas` (`supabase/schema-compras-orfas.sql`), com o payload cru preservado, e a pessoa pode se resgatar sozinha em `/vincular-compra`. Cobre os dois caminhos de perda: conta não encontrada pelo e-mail **e** plano não identificado pelo `identificarPlano` (que antes também só logava).
- **A prova de posse é o código do pedido, não só o e-mail**: exigir apenas o e-mail da compra transformaria a tela num roubo de acesso de um campo — qualquer conta criada de graça chutaria e-mails até acertar, e a resposta funcionaria como oráculo de "esse e-mail comprou?". `vincularCompraOrfa` exige `orderId` **e** `email` batendo na mesma linha; o `orderId` chega no e-mail de confirmação da Kiwify e não é adivinhável. Pedido inexistente e e-mail que não bate devolvem **resultado idêntico**, para não confirmar quais pedidos existem, e a rota tem rate limit de 5 tentativas por hora por usuário — força bruta fica inviável sem atrapalhar quem só errou a digitação.
- **RLS sem nenhuma policy, de propósito**: a tabela guarda e-mail de compra de terceiros. Uma policy de leitura para `authenticated` a transformaria em lista de compradores consultável por qualquer conta grátis. Todo acesso passa pela service role, em rota de servidor que exige login.
- **Ordem das escritas**: a ativação vem antes de marcar a linha como `vinculada`. Se o update falhar, a pessoa já está com acesso e a compra continua na fila (alguém confere de novo); o inverso apagaria da fila uma compra que ficou sem acesso. Plano não identificado **não** é chutado — vira `exige_resgate_manual`, porque liberar prazo errado é tão ruim quanto não liberar.
- **Corrigido de passagem, bug real no caminho feliz**: `buscarUserIdPorEmail` usava `.maybeSingle()` sobre uma busca `ilike`. Duas contas com o mesmo e-mail diferindo só na caixa (`Maria@x.com` e `maria@x.com`) faziam o `.maybeSingle()` tratar 2 linhas como **erro**, devolver `null` e mandar para a fila de órfãs uma compra cujo dono existe. Trocado por `.order('created_at').limit(2)`, com a conta mais antiga vencendo e a ambiguidade registrada.
...
- **Adicionado**: `src/app/robots.ts` e `src/app/sitemap.ts`. Ver ADR 041.
- **Adicionado**: `precoReais` em `PLANOS` — os valores 97 e 147 estavam cravados como números soltos dentro do webhook.
...
- **Corrigido**: `buscarUserIdPorEmail` usava `.maybeSingle()` sobre busca `ilike` — duas contas com o mesmo e-mail em caixas diferentes viravam erro, devolviam `null` e mandavam para a fila de órfãs uma compra cujo dono existe.
